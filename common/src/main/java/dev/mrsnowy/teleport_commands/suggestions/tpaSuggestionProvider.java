    ...
    public CompletableFuture<Suggestions> getSuggestions(CommandContext<CommandSourceStack> context, SuggestionsBuilder builder) {
        try {
            ...
            for (tpa.tpaArrayClass tpaEntry : playerTpaList) {
                ...
                Optional<String> recPlayerName = Optional.ofNullable(context.getSource().getServer().getPlayerList().getPlayer(UUID.fromString(tpaEntry.InitPlayer))).map(p -> p.getName().getString());
